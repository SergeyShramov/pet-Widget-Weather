const apiKey = "10207f8e3592fa7edbd68c9ba9cfa61b";
const forecastWeather = `https://api.openweathermap.org/data/2.5/forecast?q={city name}&appid=${apiKey}`;
...
window.addEventListener('load', (event) => {
    const url = forecastWeather.replace("{city name}", 'Warsaw');
    fetch(url)
...
form.addEventListener("submit", (event) => {
    ...
    if (city.length > 0) {
        const url = forecastWeather.replace("{city name}", encodeURIComponent(city));
        fetch(url)
            .then(response => response.json())
            .then((data) => {
                cityName.innerHTML = data.city.name;
